...
- T5 checkpoint complete on stacked branch `phase-enhance-02-t5-e2e-golden-paths`: added the remaining 7 Playwright specs for CSV source ingestion, graph backlink creation, AI disabled-state summarize handling, Workspace Lite side pane, Claude-format chat import, trash restore, and MCP stdio `tools/list`, bringing E2E coverage to 10 specs.
...
- T5 caveat: focused `tests/api/test_sources.py` did not complete because the local `knowledgeos_test` pytest harness hit duplicate-user/schema teardown errors after earlier failed attempts. This is tracked as a validation caveat; the source ingestion path itself passed through Docker API + worker + browser E2E.
