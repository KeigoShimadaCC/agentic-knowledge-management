...
CONTEXT7_RESULT = {
    "results": [
        {"title": "FastAPI Docs", "url": "https://fastapi.tiangolo.com", "description": "FastAPI documentation"}
    ]
}
...
async def test_answer_web_search_no_connection(auth_client: AsyncClient):
    ...
    with (
        ...
        patch("app.services.ai_service.hybrid_search", new=AsyncMock(return_value=([low_result], True))),
        patch("app.services.mcp_connection_service.find_connection_for_patterns", new=AsyncMock(return_value=None)),
    ):
        r = await auth_client.post("/api/v1/ai/answer", json={"q": "test question", "use_web_search": True})
    assert r.status_code == 200
    data = r.json()
...
async def test_answer_web_search_high_kb_score_skips_mcp(auth_client: AsyncClient):
    ...
    with (
        patch("openai.AsyncOpenAI", return_value=oai),
        patch("app.services.ai_service.hybrid_search", new=AsyncMock(return_value=([high_result], True))),
...
async def test_answer_web_search_populates_web_citations(auth_client: AsyncClient):
    ...
    with (
        patch("openai.AsyncOpenAI", return_value=oai),
        patch("app.services.ai_service.hybrid_search", new=AsyncMock(return_value=([low_result], True))),
        patch("app.services.mcp_connection_service.find_connection_for_patterns", new=AsyncMock(return_value=mock_conn)),
        patch("app.services.ai_service.McpClientSession", return_value=session_cm),
    ):
        r = await auth_client.post("/api/v1/ai/answer", json={"q": "latest AI news", "use_web_search": True})
    assert r.status_code == 200
    data = r.json()
...
async def test_answer_web_search_mcp_error_graceful(auth_client: AsyncClient):
    ...
    with (
        ...
        r = await auth_client.post("/api/v1/ai/answer", json={"q": "failing query", "use_web_search": True})
    assert r.status_code == 200
    data = r.json()
    assert data["warning"] == "web_search_unavailable"
    assert data["web_citations"] == []
...
async def test_enrich_page_wrong_user(auth_client: AsyncClient):
    ...
    # Create second user
    async with AsyncClient(transport=ASGITransport(app=fastapi_app), base_url="http://test") as other:
        await other.post(
