...
function ImportChatModal({
  ...
  async function submitPaste(event: React.FormEvent<HTMLFormElement>) {
    ...
    setIsImporting(true);
    try {
      const result = await importChatContent({
        content,
        provider,
          ...
          {(["file", "paste"] as ImportTab[]).map((value) => (
            <button
              ...
              className={clsx(
                ...
                tab === value ? "bg-gray-700 text-white" : "text-gray-400 hover:bg-gray-800 hover:text-white"
              )}
            >
          ...
          {tab === "paste" ? (
            <form onSubmit={submitPaste} className="space-y-4">
              <label className="block">
                ...
                <select
                  value={rawFormat}
                  onChange={(event) => setRawFormat(event.target.value as ChatRawFormat)}
                  className="w-full rounded-md border border-gray-700 bg-gray-950 px-3 py-2 text-sm text-white outline-none focus:border-gray-500"
                >
      ...
      {isLoading ? (
        <div className="space-y-2">
          {Array.from({ length: 6 }).map((_, index) => (
            <div key={index} className="h-20 animate-pulse rounded-lg border border-gray-800 bg-gray-900" />
          ))}
        </div>
      ) : chats.length === 0 ? (
        <div className="py-16 text-center text-gray-500">
          <p className="text-lg">No chats yet</p>
          <p className="mt-2 text-sm">Import a ChatGPT, Claude, Markdown, or text transcript</p>
        </div>
      ) : (
        <div className="space-y-2">
          {chats.map((chat) => (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="shrink-0 text-right text-xs text-gray-500">
                  ...
                  <div className="mt-1">
                    {formatDistanceToNow(new Date(chat.imported_at), { addSuffix: true })}
                  </div>
                </div>
              </div>
            </Link>
